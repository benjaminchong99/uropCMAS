...
# be equivalent to maxY - minY, the ptp sentence can be used for list and array
range_C1 = np.ptp(C1)
# print(range_C1)
...
D1 = []
for i in range(10):
    num_Inter = ((C1 >= min_C1 + i * interval) &
                 (C1 <= min_C1 + (i + 1) * interval)).sum()
    # print(num_Inter)
...
D2 = []
for i in range(10):
    num_Inter = ((C2 >= min_C2 + i * interval) &
                 (C2 <= min_C2 + (i + 1) * interval)).sum()
    # print(num_Inter)
    prob_Inter = num_Inter / len(A)
...
D3 = []
for i in range(10):
    num_Inter = ((C3 >= min_C3 + i * interval) &
                 (C3 <= min_C3 + (i + 1) * interval)).sum()
    # print(num_Inter)
    prob_Inter = num_Inter / len(A)
...
plt.plot(np.arange(0, n_step + 1, 1), o, '--ro',
         label='Nearest neighbor orientation')
...
plt.plot(h/R, Kest.poisson(h), color='green', ls=':',
         label=r'$K_{pois}$')  # poisson curve
print('3-1: y coordinate values on poisson curve:\n', Kest.poisson(h), '\n')
plt.plot(h/R, Kest(data=z, radii=h, mode='ripley'),
         '--ro', label=r'$K_{ripley}$')  # ripley curve
...
# polynomial coefficients for fitting the ripley k function curve, order=10
p = np.polyfit(h, y, 10)
dp = [10 * p[0], 9 * p[1], 8 * p[2], 7 * p[3], 6 * p[4],
      5 * p[5], 4 * p[6], 3 * p[7], 2 * p[8], 1 * p[9]]
